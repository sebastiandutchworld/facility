...
class BarcodeProductLabelsWiz(models.TransientModel):
    ...
    def print_barcode_labels(self):
        self.ensure_one()
        [data] = self.read()
        barcode_config = self.env.ref('dws_dae_barcode_labels.barcode_labels_config_data')
        ...
        return self.env.ref('dws_dae_barcode_labels.printed_barcode_labels_id').report_action(barcode_lines, data=datas)
