...
class BarcodeStockLabelsWiz(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        for order in stock_picking_ids:
            for line in order.move_line_nosuggest_ids: #move_ids_without_package:
    ...
    def print_barcode_labels(self):
        ...
        return self.env.ref('dws_dae_barcode_labels.printed_stock_picking_barcode_labels_id').report_action(barcode_lines, data=datas)
...
class BarcodeStockLabelsLine(models.TransientModel):
    ...
    lot_id = fields.Char('Lot Number')#, related='name')
    # alt_pn = fields.Char('Vendor Product Code', related='product.template.vendor_product_code')
