...
class DynamicBarcodePurchaseLabelsParser(models.AbstractModel):
	_name = 'report.dws_dae_barcode_labels.pur_dynamic_barcode_labels'
	_description = "Purchase Product variant barcode labels Report"
	...
	def _get_report_values(self, docids, data=None):
		barcode_labels_report = self.env['ir.actions.report']._get_report_from_name('dws_dae_barcode_labels.pur_dynamic_barcode_labels')
		barcode_labels = data['form']['barcode_labels']
		barcode_labels = self.env['barcode.purchase.labels.wiz.line'].browse(barcode_labels)
		return {
